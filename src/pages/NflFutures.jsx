...
export default function NflFutures() {
  const show = getShowBySlug("dancing-with-the-odds");
  const formattedDate = formatter.format(new Date(nflFutures.recordedDate));
  usePageMeta(
    "NFL Futures",
    `Dancing With the Odds' ${nflFutures.season} NFL futures picks from Dennis, Shaun, and Aaron: division champions, conference champions, and Super Bowl champion.`
  );
    ...
    <>
      <section className="text-white" style={{ background: show.colorTheme.gradient }}>
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          ...
          <h1 className="mt-1 text-3xl font-black sm:text-4xl md:text-5xl">NFL Futures</h1>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid gap-6 sm:grid-cols-3">
          {Object.entries(nflFutures.picks).map(([host, picks]) => (
            <HostFutures
              key={host}
              host={host}
              picks={picks}
              categories={nflFutures.categories}
